...
public class MessagePlus {
    ...
    /**
     * Return true if the Message has an annotation whose value contains a "+net.app.core.file"
     * key. This is helpful for determining if this MessagePlus is waiting on an full OEmbed annotation
     * to be generated by the server (i.e. it hasn't been sent yet and it has a specific file
     * associated with it).
     *
     * @return true if the Message has an annotation whose value contains a "+net.app.core.file" key,
     * false otherwise.
     */
    public boolean hasOEmbedWithReplacementValue() {
        List<Annotation> oEmbeds = mMessage.getAnnotationsOfType(Annotations.OEMBED);
        for(Annotation a : oEmbeds) {
            if(a.getValueForKey("+net.app.core.file") != null) {
                return true;
            }
        }
        return false;
    ...
    private void addOEmbedsFromAnnotations(List<Annotation> annotations) {
        for(Annotation a : annotations) {
            ...
            if(value.get("+net.app.core.file") == null) {
